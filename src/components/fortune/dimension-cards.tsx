import FeedbackButton from "./fortune-feedback";

interface Dimension {
  icon: string; iconBg: string; name: string; stars: number;
  analysis: React.ReactNode; tip: string;
}

export default function DimensionCards({ dimensions, date }: { dimensions: Dimension[]; date: string }) {
  return (
    <div className="grid grid-cols-2 gap-4">
      {dimensions.map((dim) => (
        ...
        >
          ...
          <div className="text-sm text-[var(--color-text-body)] leading-relaxed">{dim.analysis}</div>
